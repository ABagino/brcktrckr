...
interface SetRankClientProps {
  initialRows: RankedSetValueRow[]
  initialThemes: string[]
}

export default function SetRankClient({ initialRows, initialThemes }: SetRankClientProps) {
  ...
  const [rows, setRows] = useState<RankedSetValueRow[]>(initialRows)
  ...
  // Only fetch themes client-side if we don't have initial themes
  useEffect(() => {
    if (initialThemes.length > 0) return
  ...
  // Fetch data when filters change (skip initial load since we have server data)
  useEffect(() => {
    // Skip if this is the initial render with default filters
    if (isInitialLoad) {
      // Check if filters match the default (server-rendered) state
      const isDefaultState =
        mode === "most_pieces" &&
        parsedStartYear === MIN_LEGO_YEAR &&
        parsedEndYear === currentYear &&
    ...
    loadRows()
  }, [mode, parsedStartYear, parsedEndYear, selectedThemes, isInitialLoad])
  ...
  return (
    <div className={`${bodyFont.className} min-h-screen text-[#1e1e1e] dark:text-gray-100 bg-[rgb(251,249,247)] bg-[radial-gradient(circle_at_20%_0%,rgba(242,142,46,0.06)_0%,rgba(242,142,46,0)_40%),radial-gradient(circle_at_80%_10%,rgba(30,30,30,0.03)_0%,rgba(30,30,30,0)_45%)] dark:bg-gray-900 dark:bg-[radial-gradient(circle_at_15%_0%,rgba(242,142,46,0.12)_0%,rgba(242,142,46,0)_42%),radial-gradient(circle_at_85%_10%,rgba(255,255,255,0.05)_0%,rgba(255,255,255,0)_45%)]`}>
      <div className="mx-auto w-full max-w-7xl px-6 py-8 md:px-10 md:py-10">
        ...
        <div className="mb-6 rounded-2xl border-2 border-dashed border-yellow-500 dark:border-yellow-400 bg-yellow-50 dark:bg-yellow-900/20 p-5 shadow-[0_10px_30px_rgba(242,142,46,0.1)]">
          <div className="flex items-start gap-3">
            <span className="text-2xl" role="img" aria-label="construction">🚧</span>
            <div className="flex-1">
              <p className={`${headingFont.className} text-lg font-bold text-yellow-800 dark:text-yellow-300 mb-2`}>
                🧱 BRICK BY BRICK - STILL UNDER CONSTRUCTION! 👷‍♂️
              </p>
              <p className="text-sm text-yellow-700 dark:text-yellow-400 leading-relaxed">
                The <strong>Master Builders</strong> are still designing the instructions for this page! 🔧 The current formulas used here are <em>not the same</em> as the <Link href="/set-look" className="underline font-semibold hover:text-yellow-900 dark:hover:text-yellow-200">Set Search</Link>, and we want to eventually add <strong>RRP data</strong> to help you identify the sets that are 🔥 <em>HOT</em> right now. Feel free to explore, but check back later for the finished build!
              </p>
            </div>
